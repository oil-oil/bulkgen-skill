...
| 配置名 | 业务环境变量 | 系统凭据引用 |
| --- | --- | --- |
| default | `BULKGEN_API_KEY` | `bulkgen/service/default` |
...
```bash
node "$SKILL_DIR/scripts/credential-ui/src/profile.ts" run default -- node "$SKILL_DIR/scripts/generate.js" --mode solo --prompts "图片要求"
```
